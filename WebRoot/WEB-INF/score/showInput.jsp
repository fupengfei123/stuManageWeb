			...
			<form action="score?type=search" method="post">
				...
				<div
					...
					姓名：<input type="text" style="width:80px;margin-top:30px;"
						...
						class="form-control"
						style="width:120px;margin-left:170px;margin-top:-32px;">
						<option value="-1">所有班级</option>
						<%
							for (int i = 0; i < bjList.size(); i++) {
						%>
						<option value="<%=bjList.get(i).getId()%>"
							<%if (bjList.get(i).getId() == bjId) {%> selected="selected"
					...
					<div style="position:absolute;margin-left:300px;margin-top:-28px;">
						科目：<select name="subject" class="form-control"
							...
							<option value="<%=subList.get(i).getId()%>"
								<%if (subList.get(i).getId() == subId) {%> selected="selected"
		...
		<div style="margin-top: -50px;">
			<table style="width:600px;margin:190px auto;"
				...
				<tr class="info" align=center>
					...
					<td class="warning"><input style="width:100px" type="text" name="score"
						data-id="<%=sc.getId()%>" data-stuid="<%=sc.getStu().getId()%>"
						data-subid="<%=sc.getSub().getId()%>" value="<%=sc.getScore()%>" />
						<span style="width:50px;height:20px"></span>
					</td>
				</tr>
				<%
					}
		...
		<div style="position:absolute;margin-left:194px;margin-top:-180px">
			<ul class="pagination">
				...
				<li <%if (ye <= 1) {%> class="disabled" <%}%>><a
					href="score?type=showInput&page=<%=ye - 1%>">《上一页</a></li>
				<%
					int end = ye + 4;
					int begin = ye;
					if (ye + 4 >= maxPage) {
				...
				<li <%if (ye == i) {%> class="active" <%}%>><a
					href="score?type=showInput&page=<%=i%>" style="width:38px"><%=i%></a>
				</li>
				<%
					}
				%>
				<li <%if (ye >= maxPage) {%> class="disabled" <%}%>><a
					href="score?type=showInput&page=<%=ye + 1%>">下一页》</a>
				</li>
